...
export default {
  ...
  methods: {
      ...
      getColumns().then((res) => {
        ...
        if (res.status == "SUCCESS") {
          ...
          this.checkArr.forEach((item) => {
            ...
            this.defaultCheckList.push(item.label);
          });
        }
      });
    },
          ...
          if (this.form.id != undefined) {
            ...
            this.columnsOptions.forEach((item) => {
              for (let i = 0; i < this.checkList.length; i++) {
                if (this.checkList[i] == item.label) {
                  this.confirmCheckList.push(item);
                }
              }
            });
            this.form.screenColumns = this.confirmCheckList.map((item) => {
    ...
    copy(id) {
      ...
      // let url = `https://${window.location.host}/boardList?id=${id}`;
