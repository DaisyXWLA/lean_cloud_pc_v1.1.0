<template>
  <div id="numLineEcharts" style="height: 650px" />
</template>
<script>
import echarts from "echarts";
import { propsoalNumLine } from "@/api/proposal/proposal";
export default {
  data() {
    return {
      series: [],
  ...
  methods: {
    getData(deptId, beginCreateTime, endCreateTime, dateType) {
      propsoalNumLine(deptId, beginCreateTime, endCreateTime, dateType).then(
        (res) => {
    ...
    initChart() {
      let numLineChart = echarts.init(
        document.getElementById("numLineEcharts")
      );
      let option = {
        backgroundColor: "#fff",
        tooltip: {
      ...
      numLineChart.setOption(option, true);
    },
  },
};
</script>