<template>
  <div id="ideaLineEcharts" style="height: 650px" />
</template>
<script>
import echarts from "echarts";
import { proposalIdeaLine } from "@/api/proposal/proposal";
export default {
  data() {
    return {
      series: [],
  ...
  methods: {
    getData(deptId, beginCreateTime, endCreateTime, dateType) {
      proposalIdeaLine(deptId, beginCreateTime, endCreateTime, dateType).then(
        (res) => {
    ...
    initChart() {
      let ideaLineChart = echarts.init(
        document.getElementById("ideaLineEcharts")
      );
      let option = {
        backgroundColor: "#fff",
        tooltip: {
      ...
      ideaLineChart.setOption(option, true);
    },
  },
};
</script>