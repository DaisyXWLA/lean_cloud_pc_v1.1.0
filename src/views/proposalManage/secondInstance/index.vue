<template>
  <div class="app-container">
    ...
    >
      ...
      /> -->
      <el-table-column label="评审时间" align="center"
        ><template slot-scope="scope"
          ><span v-if="scope.row.resultStatus == 2" style="color: #f19066"
            >未评审</span
          ><span v-else>{{ scope.row.resultScoreTime }}</span></template
        ></el-table-column
      >
    ...
    <pagination
      ...
      @pagination="getList"
    />
    <el-drawer :title="title" :visible.sync="open" size="48%">
      <!-- <div class="header">
        <el-row>
      ...
      <div class="detail-container">
        <div class="content">
          <el-form label-width="100">
                ...
                <el-collapse-item title="提案处理状态记录">
                  <div class="process">
                    ...
                    >
                      ...
                      <div class="right">
                        <span
                          ><label>操作人：</label>{{ item.acceptName }}</span
                        >
                        <span
                          v-if="
                        ...
                        <span
                          ...
                          "
                          ><label>操作时间：</label
                          >{{ processTimeFormat(item.doneTime) }}<i v-if="index >= 1">，</i></span
                        >
                        <span v-else
                          ><label>操作时间：</label
                          >{{ processTimeFormat(item.acceptTime) }}<i v-if="index >= 1">，</i></span
                        >
            ...
            <div class="box">
              <div
                class="title check-btn"
                v-if="
                  gradingResults.length == 2 && gradingResults[1].status == 2
                "
              >
                <span>提案评审</span
                ><el-button
...
export default {
  data() {
    return {
      ...
      queryParams: {
        ...
        gradingStatus: 2,
        gradingStage: 2,
        resultBeginCreateTime: undefined,
        resultEndCreateTime: undefined,
      },
      categoryOptions: [],
      statusOptions: [
        { value: "1", title: "已评审" },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      if (this.time != null) {
        this.queryParams.resultBeginCreateTime = this.time[0];
        this.queryParams.resultEndCreateTime = this.time[1];
      }
    ...
    handleQuery() {
      ...
      this.queryParams.resultBeginCreateTime = undefined;
      this.queryParams.resultEndCreateTime = undefined;
      this.getList();
    },
    /** 重置按钮操作 */
    resetQuery() {
